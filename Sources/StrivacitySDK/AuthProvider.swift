...
public class AuthProvider {
    ...
    private init(_ issuer: URL, _ redirectUri: URL, _ clientId: String, _ storage: Storage?) {
        ...
        self.authStateManager = AuthStateManager(storage: storage ?? StorageImpl())
    }
    ...
    ///
    /// - SeeAlso:
    ///   [Redirection to RP After Logout](https://openid.net/specs/openid-connect-rpinitiated-1_0.html#RedirectionAfterLogout)
    ///
    /// - Parameters:
            ...
            self.checkAuthenticated(refreshTokenAdditionalParameters: refreshTokenAdditionalParameters) { isAuthenticated in
                ...
                if (isAuthenticated) {
                    ...
                    if let userAgent = OIDExternalUserAgentIOS(presenting: viewController) {
                        ...
                        self.currentAuthorizationFlow = OIDAuthState.authState(byPresenting: request, externalUserAgent: userAgent) { authState, error in
    ...
    /// - Parameters:
    ///     - viewController: ViewController instance for the application
    ///     - callback: this will be called after a logout (it can be successful or not). If any error happens, the error is returned in the parameter.
...
extension OIDAuthState {
    func getClaims() -> [AnyHashable: Any]? {
        if let idToken = self.lastTokenResponse?.idToken {
    ...
    func getAccessToken() -> String? {
        self.lastTokenResponse?.accessToken
    }
    
    func getConfiguration() -> OIDServiceConfiguration? {
        self.lastTokenResponse?.request.configuration ?? self.lastAuthorizationResponse.request.configuration
    }
}
